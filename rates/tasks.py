...
class RatesFetchingCoordinator(object):
    ...
    def get_already_downloaded_dates(self):
        tables = Table.objects.filter(date__gte=THRESHOLD_DATE)
...
def find_rates_to_fetch():
    ...
    for date_to_download in reversed(to_download):
        ...
        download_rates.delay(date_to_download=date_to_download, table=TABLE)
...
def download_rates(date_to_download, table):
    try:
        ...
        raw_rates = RatesDownloader(THRESHOLD_DATE).download(
            parsed.date(), table)
